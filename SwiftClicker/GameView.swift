...
//  SwiftClicker
//
//  The original clicker game, now a bonus feature alongside the autoclicker.
//
...
struct GameView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: Layout.contentSpacing) {
                ...
                .overlay { RippleOverlay(trigger: resetTick, tint: .red) }
                .keyframeAnimator(initialValue: CGFloat(1), trigger: resetTick) { view, scale in
                    view.scaleEffect(scale)
                } keyframes: { _ in
                    KeyframeTrack {
                        SpringKeyframe(0.86, duration: 0.12, spring: .snappy)
                        SpringKeyframe(1.06, duration: 0.18, spring: .bouncy)
                        SpringKeyframe(1.00, duration: 0.20, spring: .snappy)
                    }
                }
                .hoverLift(1.06)
    ...
    private func resetClicks() {
        ...
        // gear rewinds: a full backwards spin signals "reset"
